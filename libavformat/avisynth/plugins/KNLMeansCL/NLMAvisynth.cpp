...

{
    ...
    if ((info && vi.IsRGB()) || (info && vi.BitsPerComponent() != 8))
        env->ThrowError("KNLMeansCL: parameter 'info' requires Gray8 or YUVP8 color space!");
    ...
    if (vi.IsPlanar() || vi.IsRGB32() || vi.IsRGB64()) {
        if (vi.BitsPerComponent() == 8) {
            ...
            if (vi.IsRGB32() || vi.IsRGB64())
                printf("Warming: FFmpeg is just displaying the alpha channel. Compare to RGBAdjust(ab=255).\n");
        }
             ...
             )
        {
            ...
            if (vi.IsRGB32() || vi.IsRGB64())
                printf("Warming: FFmpeg is just displaying the alpha channel. Compare to RGBAdjust(ab=65535).\n");
        }
