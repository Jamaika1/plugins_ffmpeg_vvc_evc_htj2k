...
template<typename T>
static void copyPlane_sse2(const T* srcp, float* dstp, const int width, const int height, const int srcStride, const int dstStride) noexcept
{
...
template<typename T>
static void gaussianBlur_sse2(const T* __srcp, float* temp, float* dstp, const float* weightsH, const float* weightsV, const int width, const int height, const int srcStride, const int dstStride, const int radiusH, const int radiusV) noexcept
{
...
template<typename T>
static void gaussianBlurV_sse2(const T* __srcp, float* dstp, const float* weights, const int width, const int height, const int srcStride, const int dstStride, const int radius) noexcept
{
...
template<typename T>
static void gaussianBlurH_sse2(const T* _srcp, float* temp, float* dstp, const float* weights, const int width, const int height, const int srcStride, const int dstStride, const int radius) noexcept
{
    weights += radius;
...
static void detectEdge_sse2(float* blur, float* gradient, int* direction, const int width, const int height, const int stride, const int bgStride, const int mode, const int op, const float scale) noexcept
{
...
static void nonMaximumSuppression_sse2(const int* _direction, float* _gradient, float* blur, const int width, const int height, const int stride, const int bgStride, const int radiusAlign) noexcept
{
...
template<typename T>
static void binarizeCE_sse2(const float* _srcp, T* dstp, const int width, const int height, const int srcStride, const int dstStride, const int peak) noexcept
{
    for (int y{ 0 }; y < height; ++y)
    {
        for (int x{ 0 }; x < width; x += 4)
...
static void discretizeGM_sse2(const float* _srcp, T* dstp, const int width, const int height, const int srcStride, const int dstStride, const int peak) noexcept
{
    for (int y{ 0 }; y < height; ++y)
    {
        for (int x{ 0 }; x < width; x += 4)
    ...
    for (int i{ 0 }; i < planecount; ++i)
    {
        ...
        if (process[i] == 3)
        {
            ...
            if (radiusV[i] && radiusH[i])
                gaussianBlur_sse2(srcp, gradient, blur, weightsH[i].get(), weightsV[i].get(), width, height, stride, bgStride, radiusH[i], radiusV[i]);
            else if (radiusV[i])
                gaussianBlurV_sse2(srcp, blur, weightsV[i].get(), width, height, stride, bgStride, radiusV[i]);
            else if (radiusH[i])
                gaussianBlurH_sse2(srcp, gradient, blur, weightsH[i].get(), width, height, stride, bgStride, radiusH[i]);
            else
                copyPlane_sse2(srcp, blur, width, height, stride, bgStride);

            if (mode_ != -1)
            {
                detectEdge_sse2(blur, gradient, direction, width, height, stride, bgStride, mode_, op_, scale);

                if (mode_ == 0)
                {
                    nonMaximumSuppression_sse2(direction, gradient, blur, width, height, stride, bgStride, radiusAlign);
            ...
            switch (mode_)
            {
                case 0: binarizeCE_sse2(blur, dstp, width, height, bgStride, dst_stride, peak); break;
                case 1: discretizeGM_sse2(gradient, dstp, width, height, bgStride, stride, peak); break;
                default: discretizeGM_sse2<T, false>(blur, dstp, width, height, bgStride, dst_stride, peak); break;
            }
        }
        else if (process[i] == 2)
