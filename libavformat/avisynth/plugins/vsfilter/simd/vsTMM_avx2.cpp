...
template<typename T>
static inline T abs_dif_avx2(const T& a, const T& b) noexcept
{
        ...
        for (int x = 0; x < width; x += step)
        {
            ...
            switch (ttype_)
            {
                case 0: // 4 neighbors - compensated
                {
                    ...
                    const T2 atv = max((abs_dif_avx2<T2>(center, min0) + vHalf[plane]) >> vShift[plane], (abs_dif_avx2<T2>(center, max0) + vHalf[plane]) >> vShift[plane]);
                    const T2 ath = max((abs_dif_avx2<T2>(center, min1) + hHalf[plane]) >> hShift[plane], (abs_dif_avx2<T2>(center, max1) + hHalf[plane]) >> hShift[plane]);
                ...
                case 2: // 4 neighbors - not compensated
                {
                    ...
                    const T2 at = max(abs_dif_avx2<T2>(center, min0), abs_dif_avx2<T2>(center, max0));
...
void CreateMM::motionMask_avx2(PVideoFrame& src1, PVideoFrame& msk1, PVideoFrame& src2, PVideoFrame& msk2, PVideoFrame& dst, const int plane, IScriptEnvironment* env) noexcept
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x += step)
        {
            const T2 diff = abs_dif_avx2<T2>(T2().load_a(srcp1 + x), T2().load_a(srcp2 + x));
