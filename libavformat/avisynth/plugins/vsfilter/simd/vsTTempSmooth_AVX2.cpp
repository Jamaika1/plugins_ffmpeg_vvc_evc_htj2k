...
template<typename T>
AVS_FORCEINLINE static Vec8i load_avx2(const void* p)
{
...
void TTempSmooth<pfclip, fp>::filterI_avx2(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept
{
    ...
    if constexpr (std::is_same_v<T, uint8_t>)
    {
        for (int y{0}; y < height; ++y)
        {
            ...
            {
                const auto& c01{load_avx2<T>(&pfp[_maxr][x])};
                const auto& srcp_v01{load_avx2<T>(&srcp[_maxr][x])};

                const auto& c02{load_avx2<T>(&pfp[_maxr][x + 8])};
                const auto& srcp_v02{load_avx2<T>(&srcp[_maxr][x + 8])};

                const auto& c03{load_avx2<T>(&pfp[_maxr][x + 16])};
                const auto& srcp_v03{load_avx2<T>(&srcp[_maxr][x + 16])};

                const auto& c04{load_avx2<T>(&pfp[_maxr][x + 24])};
                const auto& srcp_v04{load_avx2<T>(&srcp[_maxr][x + 24])};
                ...
                if (frameIndex > fromFrame)
                {
                    ...
                    sum01 = select(Vec8fb(check_v01), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x])), weight01, sum01), sum01);
                    sum02 = select(Vec8fb(check_v02), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x + 8])), weight02, sum02), sum02);
                    sum03 = select(Vec8fb(check_v03), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x + 16])), weight03, sum03), sum03);
                    sum04 = select(Vec8fb(check_v04), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x + 24])), weight04, sum04), sum04);
                    ...
                    while (frameIndex > fromFrame)
                    {
                        ...
                        sum01 = select(Vec8fb(check_v1_01), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x])), weight01, sum01), sum01);
                        sum02 = select(Vec8fb(check_v1_02), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x + 8])), weight02, sum02), sum02);
                        sum03 = select(Vec8fb(check_v1_03), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x + 16])), weight03, sum03), sum03);
                        sum04 = select(Vec8fb(check_v1_04), mul_add(to_float(load_avx2<T>(&srcp[frameIndex][x + 24])), weight04, sum04), sum04);
                ...
                if constexpr (fp)
                {
                    compress_saturated_s2u(
                        compress_saturated(
                            truncatei(mul_add(to_float(load_avx2<T>(&srcp[_maxr][x])), (1.0f - weights01), sum01 + 0.5f)), zero_si256()),
                        zero_si256())
                        .get_low()
                        .storel(dstp + x);
                    compress_saturated_s2u(
                        compress_saturated(
                            truncatei(mul_add(to_float(load_avx2<T>(&srcp[_maxr][x + 8])), (1.0f - weights02), sum02 + 0.5f)), zero_si256()),
                        zero_si256())
                        .get_low()
                        .storel(dstp + (x + 8));
                    compress_saturated_s2u(
                        compress_saturated(
                            truncatei(mul_add(to_float(load_avx2<T>(&srcp[_maxr][x + 16])), (1.0f - weights03), sum03 + 0.5f)), zero_si256()),
                        zero_si256())
                        .get_low()
                        .storel(dstp + (x + 16));
                    compress_saturated_s2u(
                        compress_saturated(
                            truncatei(mul_add(to_float(load_avx2<T>(&srcp[_maxr][x + 24])), (1.0f - weights04), sum04 + 0.5f)), zero_si256()),
                        zero_si256())
                        .get_low()
                        .storel(dstp + (x + 24));
                }
                ...
                {
                    compress_saturated_s2u(compress_saturated(truncatei(sum01 / weights01 + 0.5f), zero_si256()), zero_si256())
                        .get_low()
                        .storel(dstp + x);
                    compress_saturated_s2u(compress_saturated(truncatei(sum02 / weights02 + 0.5f), zero_si256()), zero_si256())
                        .get_low()
                        .storel(dstp + (x + 8));
                    compress_saturated_s2u(compress_saturated(truncatei(sum03 / weights03 + 0.5f), zero_si256()), zero_si256())
                        .get_low()
                        .storel(dstp + (x + 16));
                    compress_saturated_s2u(compress_saturated(truncatei(sum04 / weights04 + 0.5f), zero_si256()), zero_si256())
                        .get_low()
                        .storel(dstp + (x + 24));
                }
            }
            ...
            for (int x{0}; x < width; x += 16)
            {
                ...
                if (frameIndex > fromFrame)
                {
                    ...
                    while (frameIndex > fromFrame)
                    {
                        ...
                        weight01 =
                            (useDiff) ? lookup<1792>((diff01 >> _shift) + v, weightSaved) : lookup<1792>(Vec8i(frameIndex), weightSaved);
                        weight02 =
                            (useDiff) ? lookup<1792>((diff02 >> _shift) + v, weightSaved) : lookup<1792>(Vec8i(frameIndex), weightSaved);
...
template void TTempSmooth<true, true>::filterI_avx2<uint8_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, false>::filterI_avx2<uint8_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, true>::filterI_avx2<uint8_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, false>::filterI_avx2<uint8_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;

template void TTempSmooth<false, true>::filterI_avx2<uint8_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, false>::filterI_avx2<uint8_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, true>::filterI_avx2<uint8_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, false>::filterI_avx2<uint8_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;

template void TTempSmooth<true, true>::filterI_avx2<uint16_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, false>::filterI_avx2<uint16_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, true>::filterI_avx2<uint16_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, false>::filterI_avx2<uint16_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;

template void TTempSmooth<false, true>::filterI_avx2<uint16_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, false>::filterI_avx2<uint16_t, true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, true>::filterI_avx2<uint16_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, false>::filterI_avx2<uint16_t, false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;

template<bool pfclip, bool fp>
template<bool useDiff>
void TTempSmooth<pfclip, fp>::filterF_avx2(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept
{
    int src_stride[15]{};
    int pf_stride[15]{};
        ...
        for (int x{0}; x < width; x += 8)
        {
            ...
            if (frameIndex > fromFrame)
            {
                ...
                while (frameIndex > fromFrame)
                {
                    ...
                    weight =
                        (useDiff) ? lookup<1792>(truncatei(diff * 255.0f) + v, weightSaved) : lookup<1792>(Vec8i(frameIndex), weightSaved);
...
template void TTempSmooth<true, true>::filterF_avx2<true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, false>::filterF_avx2<true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, true>::filterF_avx2<false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<true, false>::filterF_avx2<false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;

template void TTempSmooth<false, true>::filterF_avx2<true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, false>::filterF_avx2<true>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, true>::filterF_avx2<false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
template void TTempSmooth<false, false>::filterF_avx2<false>(
    PVideoFrame src[15], PVideoFrame pf[15], PVideoFrame& dst, const int fromFrame, const int toFrame, const int plane) noexcept;
    ...
    for (int y{0}; y < height; ++y)
    {
        for (int x{0}; x < width; x += 8)
        {
            if constexpr (std::is_integral_v<T>)
                accum += abs(to_float(load_avx2<T>(&srcp[x])) - to_float(load_avx2<T>(&srcp2[x])));
            else
