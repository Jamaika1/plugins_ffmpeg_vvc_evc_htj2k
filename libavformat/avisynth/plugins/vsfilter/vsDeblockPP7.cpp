    ...
    : GenericVideoFilter(_child), mode_(mode), y_(y), u_(u), v_(v)
{
    ...
    buffer = reinterpret_cast<int*>(_aligned_malloc(stride[0] * (vi.height + static_cast<int64_t>(16) + 8) * sizeof(int), FRAME_ALIGN));
}
...
static inline void copy_planePP7(PVideoFrame& dst, PVideoFrame& src, int plane, IScriptEnvironment* env)
{
...
PVideoFrame __stdcall vsDeblockPP7::GetFrame(int n, IScriptEnvironment* env)
{
    ...
    for (int i = 0; i < planecount; i++)
    {
        ...
        case 3:
            ...
            break;
        case 2:
            copy_planePP7(dst, src, plane, env);
            break;
        }
    }
