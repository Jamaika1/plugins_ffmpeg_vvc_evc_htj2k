    ...
    : GenericVideoFilter(_child), t_h_(t_h), t_l_(t_l), mode_(mode), op_(op), scale(scale_), process{ 0, 0, 0 }, radiusH{ 0, 0, 0 }, radiusV{ 0, 0, 0 }
{
...
#if defined(__AVX512F__)
    if (!avx512 && opt == 3)
        env->ThrowError("vsTCanny: opt=3 requires AVX512.");
#elif defined(__AVX2__)
    if (!avx2 && opt == 2)
    ...
    blur = reinterpret_cast<float*>(_aligned_malloc((pitch + radiusAlign * 2) * height * sizeof(float), alignment));
    if (!blur)
        env->ThrowError("vsTCanny: malloc failure (blur).");

    gradient = reinterpret_cast<float*>(_aligned_malloc((pitch + radiusAlign * 2) * (height + 2) * sizeof(float), alignment));
    if (!gradient)
        env->ThrowError("vsTCanny: malloc failure (gradient).");

    if (mode_ == 0)
    {
        direction = reinterpret_cast<int*>(_aligned_malloc(pitch * height * sizeof(int), alignment));
        if (!direction)
