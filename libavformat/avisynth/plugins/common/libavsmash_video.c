 ...
 *****************************************************************************/
libavsmash_video_decode_handler_t* libavsmash_video_alloc_decode_handler(void)
{
...
libavsmash_video_output_handler_t* libavsmash_video_alloc_output_handler(void)
{
    return (libavsmash_video_output_handler_t*)lw_malloc_zero(sizeof(libavsmash_video_output_handler_t));
}

void libavsmash_video_free_decode_handler(libavsmash_video_decode_handler_t* vdhp)
{
    if (!vdhp)
        return;
...
void libavsmash_video_free_output_handler(libavsmash_video_output_handler_t* vohp)
{
    if (!vohp)
        return;
...
void libavsmash_video_free_decode_handler_ptr(libavsmash_video_decode_handler_t** vdhpp)
{
    if (!vdhpp || !*vdhpp)
        return;
...
void libavsmash_video_free_output_handler_ptr(libavsmash_video_output_handler_t** vohpp)
{
    if (!vohpp || !*vohpp)
        return;
 ...
 * Setters
 *****************************************************************************/
void libavsmash_video_set_root(libavsmash_video_decode_handler_t* vdhp, lsmash_root_t* root)
{
    vdhp->root = root;
}

void libavsmash_video_set_track_id(libavsmash_video_decode_handler_t* vdhp, uint32_t track_id)
{
    vdhp->track_id = track_id;
}

void libavsmash_video_set_forward_seek_threshold(libavsmash_video_decode_handler_t* vdhp, uint32_t forward_seek_threshold)
{
    vdhp->forward_seek_threshold = forward_seek_threshold;
}

void libavsmash_video_set_seek_mode(libavsmash_video_decode_handler_t* vdhp, int seek_mode)
{
    vdhp->seek_mode = seek_mode;
}

void libavsmash_video_set_preferred_decoder_names(libavsmash_video_decode_handler_t* vdhp, const char** preferred_decoder_names)
{
    vdhp->config.preferred_decoder_names = preferred_decoder_names;
}

void libavsmash_video_set_prefer_hw_decoder(libavsmash_video_decode_handler_t* vdhp, int* prefer_hw_decoder)
{
    vdhp->config.prefer_hw_decoder = prefer_hw_decoder;
}

void libavsmash_video_set_decoder_options(libavsmash_video_decode_handler_t* vdhp, const char* ff_options)
{
    vdhp->config.ff_options = ff_options;
}

void libavsmash_video_set_log_handler(libavsmash_video_decode_handler_t* vdhp, lw_log_handler_t* lh)
{
    vdhp->config.lh = *lh;
}

void libavsmash_video_set_get_buffer_func(libavsmash_video_decode_handler_t* vdhp)
{
 ...
 * Getters
 *****************************************************************************/
lsmash_root_t* libavsmash_video_get_root(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->root : NULL;
}

uint32_t libavsmash_video_get_track_id(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->track_id : 0;
}

uint32_t libavsmash_video_get_forward_seek_threshold(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->forward_seek_threshold : 0;
}

int libavsmash_video_get_seek_mode(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->seek_mode : -1;
}

const char** libavsmash_video_get_preferred_decoder_names(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->config.preferred_decoder_names : NULL;
}

int libavsmash_video_get_error(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->config.error : -1;
}

lw_log_handler_t* libavsmash_video_get_log_handler(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? &vdhp->config.lh : NULL;
}

AVCodecContext* libavsmash_video_get_codec_context(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->config.ctx : NULL;
}

int libavsmash_video_get_max_width(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->config.prefer.width : 0;
}

int libavsmash_video_get_max_height(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->config.prefer.height : 0;
}

AVFrame* libavsmash_video_get_frame_buffer(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->frame_buffer : NULL;
}

uint32_t libavsmash_video_get_sample_count(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->sample_count : 0;
}

uint32_t libavsmash_video_get_media_timescale(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->media_timescale : 0;
}

uint64_t libavsmash_video_get_media_duration(libavsmash_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->media_duration : 0;
}

uint64_t libavsmash_video_get_min_cts(libavsmash_video_decode_handler_t* vdhp)
{
 ...
 * Fetchers
 *****************************************************************************/
static uint32_t libavsmash_video_fetch_sample_count(libavsmash_video_decode_handler_t* vdhp)
{
    if (!vdhp)
        return 0;
...
static uint32_t libavsmash_video_fetch_media_timescale(libavsmash_video_decode_handler_t* vdhp)
{
    if (!vdhp)
        return 0;
...
static uint64_t libavsmash_video_fetch_media_duration(libavsmash_video_decode_handler_t* vdhp)
{
    if (!vdhp)
        return 0;
...
int libavsmash_video_get_track(libavsmash_video_decode_handler_t* vdhp, uint32_t track_number)
{
    ...
    uint32_t track_id
        = libavsmash_get_track_by_media_type(libavsmash_video_get_root(vdhp), ISOM_MEDIA_HANDLER_TYPE_VIDEO_TRACK, track_number, lhp);
    if (track_id == 0)
        return -1;
...
int libavsmash_video_initialize_decoder_configuration(libavsmash_video_decode_handler_t* vdhp, AVFormatContext* format_ctx, int threads)
{
...
int libavsmash_video_get_summaries(libavsmash_video_decode_handler_t* vdhp)
{
    return get_summaries(vdhp->root, vdhp->track_id, &vdhp->config);
}

void libavsmash_video_force_seek(libavsmash_video_decode_handler_t* vdhp)
{
    /* Force seek before the next reading. */
    vdhp->last_sample_number = vdhp->sample_count + 1;
}

static inline uint32_t get_decoding_sample_number(order_converter_smash_t* order_converter, uint32_t composition_sample_number)
{
    return order_converter ? order_converter[composition_sample_number].composition_to_decoding : composition_sample_number;
}

uint32_t libavsmash_video_get_coded_sample_number(libavsmash_video_decode_handler_t* vdhp, uint32_t composition_sample_number)
{
    return get_decoding_sample_number(vdhp->order_converter, composition_sample_number);
}

int libavsmash_video_get_cts(libavsmash_video_decode_handler_t* vdhp, uint32_t coded_sample_number, uint64_t* cts)
{
    return lsmash_get_cts_from_media_timeline(vdhp->root, vdhp->track_id, coded_sample_number, cts);
}

int libavsmash_video_get_sample_duration(libavsmash_video_decode_handler_t* vdhp, uint32_t coded_sample_number, uint32_t* sample_duration)
{
    return lsmash_get_sample_delta_from_media_timeline(vdhp->root, vdhp->track_id, coded_sample_number, sample_duration);
}

void libavsmash_video_clear_error(libavsmash_video_decode_handler_t* vdhp)
{
    vdhp->config.error = 0;
}

void libavsmash_video_close_codec_context(libavsmash_video_decode_handler_t* vdhp)
{
...
int libavsmash_video_setup_timestamp_info(
    libavsmash_video_decode_handler_t* vdhp, libavsmash_video_output_handler_t* vohp, int64_t* framerate_num, int64_t* framerate_den)
{
    int err = -1;
    ...
    if (composition_sample_delay) {
        ...
        vdhp->order_converter = (order_converter_smash_t*)lw_malloc_zero((ts_list.sample_count + 1) * sizeof(order_converter_smash_t));
...
setup_finish:;
    if (vohp->vfr2cfr) {
        ...
        *framerate_den = (int64_t)vohp->cfr_den;
        vohp->frame_count = ((double)vohp->cfr_num / vohp->cfr_den) * ((double)media_duration / media_timescale) + 0.5;
    } else
...
static int find_random_accessible_point(
    libavsmash_video_decode_handler_t* vdhp, uint32_t composition_sample_number, uint32_t decoding_sample_number, uint32_t* rap_number)
{
    ...
    if (lsmash_get_closest_random_accessible_point_detail_from_media_timeline(
            vdhp->root, vdhp->track_id, decoding_sample_number, rap_number, &ra_flags, &number_of_leadings, &distance)
        < 0)
        *rap_number = 1;
...
static uint32_t seek_video(
    libavsmash_video_decode_handler_t* vdhp, AVFrame* picture, uint32_t composition_sample_number, uint32_t rap_number, int error_ignorance)
{
...
static int get_picture(libavsmash_video_decode_handler_t* vdhp, AVFrame* picture, uint32_t current, uint32_t goal)
{
    codec_configuration_t* config = &vdhp->config;
...
static int get_requested_picture(libavsmash_video_decode_handler_t* vdhp, AVFrame* picture, uint32_t sample_number)
{
#define MAX_ERROR_COUNT 3 /* arbitrary */
    ...
    int roll_recovery = 0;
    if (sample_number > vdhp->last_sample_number && sample_number <= vdhp->last_sample_number + vdhp->forward_seek_threshold) {
...
static uint32_t libavsmash_vfr2cfr(libavsmash_video_decode_handler_t* vdhp, libavsmash_video_output_handler_t* vohp, uint32_t sample_number)
{
    /* Convert VFR to CFR. */
    ...
    if (target_pts < current_pts) {
        for (composition_sample_number--; composition_sample_number; composition_sample_number--) {
    ...
    for (composition_sample_number++; composition_sample_number <= vdhp->sample_count; composition_sample_number++) {
        uint32_t decoding_sample_number = get_decoding_sample_number(vdhp->order_converter, composition_sample_number);
        if (lsmash_get_sample_info_from_media_timeline(vdhp->root, vdhp->track_id, decoding_sample_number, &sample) < 0)
            return 0;
        current_pts = (double)(sample.cts - vdhp->min_cts) / vdhp->media_timescale;
...
int libavsmash_video_get_frame(libavsmash_video_decode_handler_t* vdhp, libavsmash_video_output_handler_t* vohp, uint32_t sample_number)
{
    if (vohp->vfr2cfr) {
...
int libavsmash_video_find_first_valid_frame(libavsmash_video_decode_handler_t* vdhp)
{
...
int libavsmash_video_create_keyframe_list(libavsmash_video_decode_handler_t* vdhp)
{
    ...
    for (uint32_t composition_sample_number = 1; composition_sample_number <= vdhp->sample_count; composition_sample_number++) {
        uint32_t decoding_sample_number = get_decoding_sample_number(vdhp->order_converter, composition_sample_number);
        uint32_t rap_number;
        if (lsmash_get_closest_random_accessible_point_from_media_timeline(vdhp->root, vdhp->track_id, decoding_sample_number, &rap_number)
            < 0)
            continue;
...
int libavsmash_video_is_keyframe(libavsmash_video_decode_handler_t* vdhp, libavsmash_video_output_handler_t* vohp, uint32_t sample_number)
{
    if (vohp->vfr2cfr)
        sample_number = libavsmash_vfr2cfr(vdhp, vohp, sample_number);
