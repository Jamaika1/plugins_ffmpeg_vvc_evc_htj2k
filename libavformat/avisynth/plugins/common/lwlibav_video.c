 ...
 *****************************************************************************/
lwlibav_video_decode_handler_t* lwlibav_video_alloc_decode_handler(void)
{
...
lwlibav_video_output_handler_t* lwlibav_video_alloc_output_handler(void)
{
    return (lwlibav_video_output_handler_t*)lw_malloc_zero(sizeof(lwlibav_video_output_handler_t));
}

void lwlibav_video_free_decode_handler(lwlibav_video_decode_handler_t* vdhp)
{
    if (!vdhp)
        return;
...
void lwlibav_video_free_output_handler(lwlibav_video_output_handler_t* vohp)
{
    if (!vohp)
        return;
...
void lwlibav_video_free_decode_handler_ptr(lwlibav_video_decode_handler_t** vdhpp)
{
    if (!vdhpp || !*vdhpp)
        return;
...
void lwlibav_video_free_output_handler_ptr(lwlibav_video_output_handler_t** vohpp)
{
    if (!vohpp || !*vohpp)
        return;
 ...
 * Setters
 *****************************************************************************/
void lwlibav_video_set_forward_seek_threshold(lwlibav_video_decode_handler_t* vdhp, uint32_t forward_seek_threshold)
{
    vdhp->forward_seek_threshold = forward_seek_threshold;
}

void lwlibav_video_set_seek_mode(lwlibav_video_decode_handler_t* vdhp, int seek_mode)
{
    vdhp->seek_mode = seek_mode;
}

void lwlibav_video_set_preferred_decoder_names(lwlibav_video_decode_handler_t* vdhp, const char** preferred_decoder_names)
{
    vdhp->preferred_decoder_names = preferred_decoder_names;
}

void lwlibav_video_set_prefer_hw_decoder(lwlibav_video_decode_handler_t* vdhp, int* prefer_hw_decoder)
{
    vdhp->prefer_hw_decoder = prefer_hw_decoder;
}

void lwlibav_video_set_decoder_options(lwlibav_video_decode_handler_t* vdhp, const char* ff_options)
{
    vdhp->ff_options = ff_options;
}

void lwlibav_video_set_log_handler(lwlibav_video_decode_handler_t* vdhp, lw_log_handler_t* lh)
{
    vdhp->lh = *lh;
}

void lwlibav_video_set_get_buffer_func(lwlibav_video_decode_handler_t* vdhp)
{
 ...
 * Getters
 *****************************************************************************/
const char** lwlibav_video_get_preferred_decoder_names(lwlibav_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->preferred_decoder_names : NULL;
}

int lwlibav_video_get_error(lwlibav_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->error : -1;
}

lw_log_handler_t* lwlibav_video_get_log_handler(lwlibav_video_decode_handler_t* vdhp)
{
    return vdhp ? &vdhp->lh : NULL;
}

AVCodecContext* lwlibav_video_get_codec_context(lwlibav_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->ctx : NULL;
}

int lwlibav_video_get_max_width(lwlibav_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->max_width : 0;
}

int lwlibav_video_get_max_height(lwlibav_video_decode_handler_t* vdhp)
{
    return vdhp ? vdhp->max_height : 0;
}

AVFrame* lwlibav_video_get_frame_buffer(lwlibav_video_decode_handler_t* vdhp)
{
 ...
 * Others
 *****************************************************************************/
void lwlibav_video_force_seek(lwlibav_video_decode_handler_t* vdhp)
{
    /* Force seek before the next reading. */
    vdhp->last_frame_number = vdhp->frame_count + 1;
}

int lwlibav_video_get_desired_track(const char* file_path, lwlibav_video_decode_handler_t* vdhp, int threads)
{
    AVCodecContext* ctx = NULL;
    if (vdhp->stream_index < 0 || vdhp->frame_count == 0 || lavf_open_file(&vdhp->format, file_path, &vdhp->lh) < 0
        || find_and_open_decoder(&ctx, vdhp->format->streams[vdhp->stream_index]->codecpar, vdhp->preferred_decoder_names,
               vdhp->prefer_hw_decoder, threads, -1.0, vdhp->ff_options, vdhp->hw_device_ctx)
            < 0) {
...
void lwlibav_video_setup_timestamp_info(lwlibav_file_handler_t* lwhp, lwlibav_video_decode_handler_t* vdhp,
    lwlibav_video_output_handler_t* vohp, int64_t* framerate_num, int64_t* framerate_den, int apply_repeat_flag)
{
    ...
    if (vdhp->frame_count == 1 || lwhp->raw_demuxer || vdhp->actual_time_base.num == 0 || vdhp->actual_time_base.den == 0
    ...
    double stream_framerate = (vohp->frame_count - (vohp->repeat_correction_ts ? 1 : 0)) * ((double)stream_timescale / stream_duration);
...
use_lavf_frame_rate:
    if (!(stream->avg_frame_rate.num && stream->avg_frame_rate.den)
        || (stream->avg_frame_rate.den != 1 && stream->avg_frame_rate.den != 1001
            && (stream->r_frame_rate.den == 1 || stream->r_frame_rate.den == 1001))) {
...
void lwlibav_video_set_initial_input_format(lwlibav_video_decode_handler_t* vdhp)
{
...
static inline void set_output_order_id(lwlibav_video_decode_handler_t* vdhp, AVPacket* pkt, uint32_t coded_picture_number)
{
...
static inline int64_t get_output_order_id(AVFrame* decoded_frame_buffer)
{
    return decoded_frame_buffer->pts;
}

static uint32_t correct_current_frame_number(lwlibav_video_decode_handler_t* vdhp, AVPacket* pkt, uint32_t i, /* picture_number */
    uint32_t goal)
{
/* Match packet position if available and enabled. */
#define MATCH_POS(j) \
    ((vdhp->lw_seek_flags & SEEK_POS_CORRECTION) && pkt->pos != -1 && info[j].file_offset != -1 && info[j].file_offset == pkt->pos)
...
static int decode_video_picture(lwlibav_video_decode_handler_t* vdhp, AVFrame* frame, int* got_picture, int64_t* pkt_pts, uint32_t* current,
    uint32_t goal, uint32_t rap_number)
{
    ...
    do {
        ...
        if (!vdhp->reuse_pkt) {
            ...
            if (picture_number == rap_number && (vdhp->lw_seek_flags & (SEEK_DTS_BASED | SEEK_PTS_BASED))) {
                ...
                if (picture_number == 0 || picture_number > rap_number)
                    return -2;
...
static void find_random_accessible_point(
    lwlibav_video_decode_handler_t* vdhp, uint32_t presentation_picture_number, uint32_t decoding_picture_number, uint32_t* rap_number)
{
    ...
    *rap_number = decoding_picture_number;
    while (*rap_number > 1) {
        int is_bad = 0;
        for (uint32_t i = 0; i < vdhp->bad_seek_point_count; ++i) {
            if (vdhp->bad_seek_points[i] == *rap_number) {
                is_bad = 1;
                break;
            }
        }
        if (vdhp->keyframe_list[*rap_number] && !is_bad) {
            if (!is_leading)
                break;
...
static int64_t get_random_accessible_point_position(lwlibav_video_decode_handler_t* vdhp, uint32_t rap_number)
{
    uint32_t presentation_rap_number = vdhp->order_converter ? vdhp->order_converter[rap_number].decoding_to_presentation : rap_number;
...
static inline uint32_t is_half_frame(lwlibav_video_decode_handler_t* vdhp, uint32_t output_picture_number)
{
    return (output_picture_number <= vdhp->frame_count && vdhp->frame_list[output_picture_number].repeat_pict == 0);
}

static void correct_output_delay(lwlibav_video_decode_handler_t* vdhp, uint32_t* goal_fed_picture_number, uint32_t reliable_picture_number,
 ...
 * adjustments.
 */
static int lavf_seek_frame(AVFormatContext* s, int stream_index, int64_t timestamp, int flags)
{
...
static uint32_t seek_video(lwlibav_video_decode_handler_t* vdhp, AVFrame* frame, uint32_t presentation_picture_number, uint32_t rap_number,
    int64_t rap_pos, int error_ignorance)
{
    ...
    int verification_done = 0; // Flag to ensure we only verify the first decoded frame.
    exhp->delay_count = 0;
    ...
    for (current = rap_number; current <= goal; current++) {
        ...
        if (got_picture) {
            if (!verification_done) {
                verification_done = 1; // Mark verification as done for this seek.
        ...
        else if (current <= vdhp->frame_count && current >= rap_number + decoder_delay && vdhp->frame_list[current].repeat_pict == 0) {
...
static inline int copy_last_req_frame(lwlibav_video_decode_handler_t* vdhp, AVFrame* frame)
{
...
static int is_picture_stored_in_frame(lwlibav_video_decode_handler_t* vdhp, AVFrame* frame, uint32_t picture_number)
{
    if (!frame)
        return -1;
    int64_t output_id = get_output_order_id(frame);
...
static int get_frame(lwlibav_video_decode_handler_t* vdhp, AVFrame* frame, /* requesting frame buffer */
    uint32_t current, uint32_t requested_picture_number, uint32_t rap_number)
{
...
static inline uint32_t get_last_half_offset(lwlibav_video_decode_handler_t* vdhp)
{
    if (!vdhp->last_half_frame)
        return 0;
    int field_number = field_number_of_picture_in_frame(vdhp, vdhp->last_req_frame, vdhp->last_frame_number);
    return field_number == 1 ? 1 : field_number == 2 ? -1 : 0;
}

static int get_requested_picture(lwlibav_video_decode_handler_t* vdhp, AVFrame* frame, uint32_t picture_number)
{
#define MAX_ERROR_COUNT 3 /* arbitrary */
    ...
    if (picture_number == vdhp->last_frame_number || picture_number == vdhp->last_frame_number + last_half_offset) {
    ...
    if (picture_number > last_frame_number && picture_number <= last_frame_number + vdhp->forward_seek_threshold) {
        start_number = vdhp->last_fed_picture_number + 1;
        rap_number = vdhp->last_rap_number;
    } else {
        uint32_t search_start_point = picture_number;
        while (1) // Loop until we succeed or run out of options.
        {
            // Find the nearest valid (and not-yet-failed) RAP at or before our current search point.
            find_random_accessible_point(vdhp, search_start_point, 0, &rap_number);
            if (rap_number == 0 || (search_start_point > 1 && rap_number == 1 && vdhp->bad_seek_point_count > 0)) {
                // We've searched all the way to the beginning and found no working RAP.
                goto video_fail;
            }
            // If we are stuck trying the same RAP again, something is wrong.
            if (rap_number == vdhp->last_rap_number && picture_number > last_frame_number) {
                start_number = vdhp->last_fed_picture_number + 1;
                break; // Fall through to sequential decode.
            }
            ...
            if (start_number > 0) {
                // Success! seek_video verified the RAP and returned a valid start point.
                break;
            }
            // Verification failed. Mark this RAP as bad.
            uint32_t new_count = vdhp->bad_seek_point_count + 1;
            uint32_t* new_list = (uint32_t*)realloc(vdhp->bad_seek_points, new_count * sizeof(uint32_t));
            if (new_list) {
                vdhp->bad_seek_points = new_list;
                vdhp->bad_seek_points[vdhp->bad_seek_point_count] = rap_number;
                vdhp->bad_seek_point_count = new_count;
            }
            // Set the next search to start from the frame just before the failed RAP.
            if (rap_number > 1) {
                search_start_point = rap_number - 1;
            } else {
                // We tried RAP 1 and it failed. There's nowhere else to go.
                goto video_fail;
            }
        }
    }
    ...
    int error_count = 0;
    while (start_number == 0 || get_frame(vdhp, frame, start_number, picture_number, rap_number) < 0) {
...
static inline int check_frame_buffer_identical(AVFrame* a, AVFrame* b)
{
    return !memcmp(a->data, b->data, sizeof(a->data)) && !memcmp(a->linesize, b->linesize, sizeof(a->linesize));
}

static inline int copy_frame(lw_log_handler_t* lhp, AVFrame* dst, AVFrame* src)
{
    av_frame_unref(dst);
...
static inline int copy_field(lw_log_handler_t* lhp, AVFrame* dst, AVFrame* src, int line_offset, int top_field_first)
{
    ...
    if (av_frame_is_writable(dst) == 0) {
        ...
        for (int i = 0; i < number_of_planes; i++) {
            ...
            av_image_copy_plane(dst->data[i] + dst->linesize[i] * line_offset, 2 * dst->linesize[i],
                src->data[i] + src->linesize[i] * line_offset, 2 * src->linesize[i], MIN(dst->linesize[i], src->linesize[i]), field_height);
        }
    }
...
static int lwlibav_repeat_control(lwlibav_video_decode_handler_t* vdhp, lwlibav_video_output_handler_t* vohp, uint32_t frame_number)
{
    ...
    if (repeat_control == REPEAT_CONTROL_DECODE_BOTH_FIELDS) {
        ...
        int decode_number = repeat_control == REPEAT_CONTROL_DECODE_ONE_FRAME ? first_field_number
            : repeat_control == REPEAT_CONTROL_DECODE_TOP_FIELD               ? t
                                                                              : b;
...
static int64_t lwlibav_get_ts(lwlibav_video_decode_handler_t* vdhp, uint32_t frame_number)
{
...
static uint32_t lwlibav_vfr2cfr(lwlibav_video_decode_handler_t* vdhp, lwlibav_video_output_handler_t* vohp, uint32_t frame_number)
{
    ...
    if (target_ts < current_ts) {
        for (composition_frame_number--; composition_frame_number; composition_frame_number--) {
    ...
    for (composition_frame_number++; composition_frame_number <= vdhp->frame_count; composition_frame_number++) {
        ts = lwlibav_get_ts(vdhp, composition_frame_number);
        if (ts != AV_NOPTS_VALUE) {
            current_ts = ((double)(ts - vdhp->min_ts) * time_base.num) / time_base.den;
            if (current_ts >= target_ts) {
...
static void handle_decoder_pix_fmt(AVCodecParameters* codecpar, const AVCodec* codec, enum AVPixelFormat pix_fmt)
{
...
static int get_video_frame(lwlibav_video_decode_handler_t* vdhp, lwlibav_video_output_handler_t* vohp, uint32_t frame_number)
{
...
int lwlibav_video_get_frame(lwlibav_video_decode_handler_t* vdhp, lwlibav_video_output_handler_t* vohp, uint32_t frame_number)
{
    if (vohp->vfr2cfr) {
...
int lwlibav_video_is_keyframe(lwlibav_video_decode_handler_t* vdhp, lwlibav_video_output_handler_t* vohp, uint32_t frame_number)
{
    assert(frame_number);
    if (vohp->vfr2cfr)
        frame_number = lwlibav_vfr2cfr(vdhp, vohp, frame_number);
...
int lwlibav_video_find_first_valid_frame(lwlibav_video_decode_handler_t* vdhp)
{
    ...
    vdhp->av_seek_flags = (vdhp->lw_seek_flags & SEEK_POS_BASED) ? AVSEEK_FLAG_BYTE : vdhp->lw_seek_flags == 0 ? AVSEEK_FLAG_FRAME : 0;
    ...
    for (uint32_t i = 1; i <= vdhp->frame_count + vdhp->exh.delay_count; i++) {
        ...
        /* Handle decoder delay derived from PAFF field coded pictures. */
        if (i <= vdhp->frame_count && i > decoder_delay && !got_picture && vdhp->frame_list[i].repeat_pict == 0) {
...
enum lw_field_info lwlibav_video_get_field_info(lwlibav_video_decode_handler_t* vdhp, uint32_t frame_number)
{
    return frame_number <= vdhp->frame_count ? vdhp->frame_list[frame_number].field_info : LW_FIELD_INFO_UNKNOWN;
}
