...
LWLibavVideoSource::LWLibavVideoSource(lwlibav_option_t* opt, int seek_mode, uint32_t forward_seek_threshold, int direct_rendering,
    int64_t fps_num, int64_t fps_den, enum AVPixelFormat pixel_format, const char* preferred_decoder_names, int prefer_hw_decoder,
    bool progress, const char* ff_options, IScriptEnvironment* env)
    : LWLibavVideoSource {}
{
...
AVSValue __cdecl CreateLWLibavVideoSource(AVSValue args, void* user_data, IScriptEnvironment* env)
{
    ...
    int ff_loglevel = args[15].AsInt(5);
    ...
    opt.vfr2cfr.active = 1; //fps_num > 0 && fps_den > 0 ? 1 : 0;
    ...
    return new LWLibavVideoSource(&opt, seek_mode, forward_seek_threshold, direct_rendering, fps_num, fps_den, pixel_format, preferred_decoder_names,
...
AVSValue __cdecl CreateLWLibavAudioSource(AVSValue args, void* user_data, IScriptEnvironment* env)
{
    ...
    int ff_loglevel = args[8].AsInt(5);
