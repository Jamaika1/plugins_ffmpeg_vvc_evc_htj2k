...
#include "except.h"
#include "zassert.h"
#include "libgraphengine/graph.h"
...
unsigned FilterGraph::get_input_buffering() const try
{
	...
	auto it = std::find_if(buffering.begin(), buffering.end(), [=, this](const auto &entry) { return entry.id == m_source_id; });
#else
...
unsigned FilterGraph::get_output_buffering() const try
{
	graphengine::Graph::BufferingRequirement buffering = m_graph->get_buffering_requirement();
#if __MINGW32__ && (__cplusplus > 201703L)
	auto it = std::find_if(buffering.begin(), buffering.end(), [=, this](const auto &entry) { return entry.id == m_sink_id; });
#else
