...
void svt_av1_highbd_jnt_convolve_2d_copy_sse4_1(
    const uint16_t *src, int32_t src_stride, uint16_t *dst0, int32_t dst_stride0, int32_t w,
    int32_t h, const InterpFilterParams *filter_params_x, const InterpFilterParams *filter_params_y,
    const int32_t subpel_x_q4, const int32_t subpel_y_q4, ConvolveParams *conv_params, int32_t bd) {
    if (w <= 4) {
    ...
    if (!(w % 8)) {
        for (i = 0; i < h; i += 1) {
            for (j = 0; j < w; j += 8) {
                const __m128i src_16bit = _mm_loadu_si128((__m128i *)(&src[i * src_stride + j]));
                ...
                if (do_average) {
                    ...
                    const __m128i res_unsigned_lo = _mm_add_epi32(res_32b_lo, offset_const);
                    ...
                    const __m128i res_unsigned_hi = _mm_add_epi32(res_32b_hi, offset_const);
                    ...
                    const __m128i res_unsigned_16b = _mm_adds_epu16(res, offset_const_16b);

                    _mm_storeu_si128((__m128i *)(&dst[i * dst_stride + j]), res_unsigned_16b);
                }
            }
        }
    } else if (!(w % 4)) {
        for (i = 0; i < h; i += 2) {
            for (j = 0; j < w; j += 4) {
                const __m128i src_row_0 = _mm_loadl_epi64((__m128i *)(&src[i * src_stride + j]));
                ...
                if (do_average) {
                    ...
                    _mm_storel_epi64((__m128i *)(&dst[i * dst_stride + j]), res_unsigned_16b);
...
void svt_av1_highbd_jnt_convolve_2d_sse4_1(const uint16_t *src, int32_t src_stride, uint16_t *dst16,
    ...
    /* Horizontal filter */
    {
        ...
        for (i = 0; i < im_h; ++i) {
            for (j = 0; j < w; j += 8) {
                ...
                const __m128i res_2 = _mm_madd_epi16(_mm_alignr_epi8(data2, data, 4), coeff_23);
                ...
                const __m128i res_3 = _mm_madd_epi16(_mm_alignr_epi8(data2, data, 6), coeff_23);
    ...
    /* Vertical filter */
    {
        ...
        for (i = 0; i < h; ++i) {
            for (j = 0; j < w; j += 8) {
                ...
                const __m128i res_unsigned_lo = _mm_add_epi32(res_lo_round, offset_const);

                if (w < 8) {
                    if (do_average) {
                        const __m128i data_0 = _mm_loadl_epi64(
                        ...
                        _mm_storel_epi64((__m128i *)(&dst16[i * dst16_stride + j]), res_clip);
                    } else {
                        const __m128i res_16b = _mm_packus_epi32(res_unsigned_lo, res_unsigned_lo);
                    ...
                    const __m128i res_unsigned_hi = _mm_add_epi32(res_hi_round, offset_const);

                    if (do_average) {
                        const __m128i data_lo = _mm_loadl_epi64(
                            (__m128i *)(&dst[i * dst_stride + j]));
                        const __m128i data_hi = _mm_loadl_epi64(
                            (__m128i *)(&dst[i * dst_stride + j + 4]));
                        ...
                        _mm_storeu_si128((__m128i *)(&dst16[i * dst16_stride + j]), res_clip);
                    } else {
                        const __m128i res_16b = _mm_packus_epi32(res_unsigned_lo, res_unsigned_hi);
