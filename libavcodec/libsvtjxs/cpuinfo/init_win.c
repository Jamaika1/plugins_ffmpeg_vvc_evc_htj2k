...
BOOL CALLBACK cpuinfo_x86_windows_init(PINIT_ONCE init_once, PVOID parameter, PVOID* context) {
	...
	if (processors == NULL) {
		cpuinfo_log_error(
			"failed to allocate %llu bytes for descriptions of %" PRIu32 " logical processors",
	...
	if (cores == NULL) {
		cpuinfo_log_error(
			"failed to allocate %llu bytes for descriptions of %" PRIu32 " cores",
	...
	if (clusters == NULL) {
		cpuinfo_log_error(
			"failed to allocate %llu bytes for descriptions of %" PRIu32 " core clusters",
	...
	if (packages == NULL) {
		cpuinfo_log_error(
			"failed to allocate %llu bytes for descriptions of %" PRIu32 " physical packages",
	...
	if (l1i_count != 0) {
		...
		if (l1i == NULL) {
			cpuinfo_log_error(
				"failed to allocate %llu bytes for descriptions of %" PRIu32 " L1I caches",
	...
	if (l1d_count != 0) {
		...
		if (l1d == NULL) {
			cpuinfo_log_error(
				"failed to allocate %llu bytes for descriptions of %" PRIu32 " L1D caches",
	...
	if (l2_count != 0) {
		...
		if (l2 == NULL) {
			cpuinfo_log_error(
				"failed to allocate %llu bytes for descriptions of %" PRIu32 " L2 caches",
	...
	if (l3_count != 0) {
		...
		if (l3 == NULL) {
			cpuinfo_log_error(
				"failed to allocate %llu bytes for descriptions of %" PRIu32 " L3 caches",
	...
	if (l4_count != 0) {
		...
		if (l4 == NULL) {
			cpuinfo_log_error(
				"failed to allocate %llu bytes for descriptions of %" PRIu32 " L4 caches",
