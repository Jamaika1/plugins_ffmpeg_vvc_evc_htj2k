...
struct FSK * fsk_create_core(int Fs, int Rs, int M, int P, int Nsym, int f1_tx, int tone_spacing)
{
    ...
    fsk->fft_cfg = codec2_kiss_fft_alloc(Ndft,0,NULL,NULL); assert(fsk->fft_cfg != NULL);
...
void fsk_demod_freq_est(struct FSK *fsk, COMP fsk_in[], float *freqs, int M) {
    ...
    /* Array to do complex FFT from using codec2_kiss_fft */
    #ifdef DEMOD_ALLOC_STACK
    codec2_kiss_fft_cpx *fftin  = (codec2_kiss_fft_cpx*)alloca(sizeof(codec2_kiss_fft_cpx)*Ndft);
    codec2_kiss_fft_cpx *fftout = (codec2_kiss_fft_cpx*)alloca(sizeof(codec2_kiss_fft_cpx)*Ndft);
    #else
