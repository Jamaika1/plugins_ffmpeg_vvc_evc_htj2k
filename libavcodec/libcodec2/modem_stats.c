...
void modem_stats_open(struct MODEM_STATS *f)
{
	...
	f->fft_buf[i] = 0.0;
    f->fft_cfg = (void*)codec2_kiss_fft_alloc (2*MODEM_STATS_NSPEC, 0, NULL, NULL);
...
void modem_stats_get_rx_spectrum(struct MODEM_STATS *f, float mag_spec_dB[], COMP rx_fdm[], int nin)
{
    ...
    codec2_kiss_fft((codec2_kiss_fft_cfg)f->fft_cfg, (codec2_kiss_fft_cpx *)fft_in, (codec2_kiss_fft_cpx *)fft_out);
