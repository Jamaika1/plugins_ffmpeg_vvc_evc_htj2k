...
#include "libsvtjxs/SvtJpegxsEnc.h"
...
static void set_bpp(const char* value, svt_jpeg_xs_encoder_api_t* encoder) {
    ...
    printf("bpp numerator %d", (int)encoder->bpp_numerator);
}
...
static av_cold int svt_jpegxs_enc_init(AVCodecContext* avctx) {
    ...
    svt_jpeg_xs_encoder_api_t* encoder;
    if (!svt_enc->bpp_str) {
      svt_enc->encoder.bpp_numerator = (uint32_t)(avctx->pix_fmt == AV_PIX_FMT_GRAY8 ? 8 :
                                                 (avctx->pix_fmt == AV_PIX_FMT_YUV420P ? 12 :
                                                 (avctx->pix_fmt == AV_PIX_FMT_YUV422P ? 16 : 24)));
      //svt_enc->bpp_str = (char*)((int)desc->comp[0].depth);
    } else {
