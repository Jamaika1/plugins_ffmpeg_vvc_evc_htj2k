...
void ldlHorizontalU8PlanarNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                               ...
                               uint32_t xEnd, const Kernel_t* kernel)
{
    ...
    ldlUpscaleHorizontalGetCoords(width, xStart, xEnd, kernelLength, UCHoriLoadAlignment, &coords);

    /* Run left edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsLeftValid(&coords)) {
        ldlHorizontalU8Planar(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel);
    }
    ...
    if (ldlUpscaleHorizontalCoordsIsRightValid(&coords)) {
        ldlHorizontalU8Planar(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel);
    }
}

/*! \brief Planar horizontal upscaling of 2 rows. */
void ldlHorizontalS16PlanarNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                                const uint8_t* base[2], uint32_t width, uint32_t xStart,
                                uint32_t xEnd, const Kernel_t* kernel)
{
    const int16_t* kernelFwd = kernel->coeffs[0];
    ...
    /* Run left edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsLeftValid(&coords)) {
        ldlHorizontalS16Planar(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsRightValid(&coords)) {
        ldlHorizontalS16Planar(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel);
    }
}

/*! \brief Planar horizontal upscaling of 2 rows. */
static inline void ldlHorizontalU16PlanarNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                                              const uint8_t* base[2], uint32_t width,
                                              uint32_t xStart, uint32_t xEnd, const Kernel_t* kernel,
                                              uint16_t maxValue, bool is14Bit)
{
    ...
    /* Run left edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsLeftValid(&coords)) {
        ldlHorizontalUNPlanar(dither, in, out, base, width, coords.leftStart, coords.leftEnd,
                              kernel, maxValue);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsRightValid(&coords)) {
        ldlHorizontalUNPlanar(dither, in, out, base, width, coords.rightStart, coords.rightEnd,
                              kernel, maxValue);
    }
}

/*! \brief U10 Planar horizontal upscaling of 2 rows. */
void ldlHorizontalU10PlanarNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                                const uint8_t* base[2], uint32_t width, uint32_t xStart,
                                uint32_t xEnd, const Kernel_t* kernel)
{
    ldlHorizontalU16PlanarNEON(dither, in, out, base, width, xStart, xEnd, kernel, 1023, false);
}

/*! \brief U12 Planar horizontal upscaling of 2 rows. */
void ldlHorizontalU12PlanarNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                                const uint8_t* base[2], uint32_t width, uint32_t xStart,
                                uint32_t xEnd, const Kernel_t* kernel)
{
    ldlHorizontalU16PlanarNEON(dither, in, out, base, width, xStart, xEnd, kernel, 4095, false);
}

/*! \brief U14 Planar horizontal upscaling of 2 rows. */
void ldlHorizontalU14PlanarNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                                const uint8_t* base[2], uint32_t width, uint32_t xStart,
                                uint32_t xEnd, const Kernel_t* kernel)
{
    ldlHorizontalU16PlanarNEON(dither, in, out, base, width, xStart, xEnd, kernel, 16383, true);
}

/*! \brief NV12 horizontal upscaling of 2 rows. */
void ldlHorizontalU8NV12NEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                             ...
                             const Kernel_t* kernel)
{
    ...
    /* Determine edge-cases that should be run in non-SIMD codepath. */
    ldlUpscaleHorizontalGetCoords(width, xStart, xEnd, kernelLength, UCHoriLoadAlignmentNV12, &coords);

    /* Run left edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsLeftValid(&coords)) {
        ldlHorizontalU8NV12(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsRightValid(&coords)) {
        ldlHorizontalU8NV12(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel);
    }
}

/*! \brief RGB horizontal upscaling of 2 rows. */
void ldlHorizontalU8RGBNEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                            const uint8_t* base[2], uint32_t width, uint32_t xStart, uint32_t xEnd,
                            const Kernel_t* kernel)
{
    const int16_t* kernelFwd = kernel->coeffs[0];
    ...
    /* Determine edge-cases that should be run in non-SIMD codepath. */
    ldlUpscaleHorizontalGetCoords(width, xStart, xEnd, kernelLength, UCHoriLoadAlignmentRGB, &coords);

    /* Run left edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsLeftValid(&coords)) {
        ldlHorizontalU8RGB(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsRightValid(&coords)) {
        ldlHorizontalU8RGB(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel);
    }
}

/*! \brief RGBA horizontal upscaling of 2 rows. */
void ldlHorizontalU8RGBANEON(Dither_t dither, const uint8_t* in[2], uint8_t* out[2],
                             const uint8_t* base[2], uint32_t width, uint32_t xStart, uint32_t xEnd,
                             const Kernel_t* kernel)
{
    uint32_t x;
    ...
    /* Determine edge-cases that should be run in non-SIMD codepath. */
    ldlUpscaleHorizontalGetCoords(width, xStart, xEnd, kernelLength, UCHoriLoadAlignmentRGBA, &coords);

    /* Run left edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsLeftValid(&coords)) {
        ldlHorizontalU8RGBA(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (ldlUpscaleHorizontalCoordsIsRightValid(&coords)) {
        ldlHorizontalU8RGBA(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel);
    }
}
...
static void ldlVerticalU8NEON(const uint8_t* in, uint32_t inStride, uint8_t* out, uint32_t outStride,
...
static void ldlVerticalS16NEON(const uint8_t* in, uint32_t inStride, uint8_t* out, uint32_t outStride,
                               uint32_t y, uint32_t rows, uint32_t height, const Kernel_t* kernel)
{
    const int16_t* kernelFwd = kernel->coeffs[0];
    const int16_t* kernelRev = kernel->coeffs[1];
...
static inline void ldlVerticalU16NEON(const uint8_t* in, uint32_t inStride, uint8_t* out,
...
void ldlVerticalU10NEON(const uint8_t* in, uint32_t inStride, uint8_t* out, uint32_t outStride,
                        uint32_t y, uint32_t rows, uint32_t height, const Kernel_t* kernel)
{
    ldlVerticalU16NEON(in, inStride, out, outStride, y, rows, height, kernel, 1023);
}

/*! \brief U12 vertical upscaling of 16 columns. */
void ldlVerticalU12NEON(const uint8_t* in, uint32_t inStride, uint8_t* out, uint32_t outStride,
                        uint32_t y, uint32_t rows, uint32_t height, const Kernel_t* kernel)
{
    ldlVerticalU16NEON(in, inStride, out, outStride, y, rows, height, kernel, 4095);
}

/*! \brief U14 vertical upscaling of 16 columns. */
void ldlVerticalU14NEON(const uint8_t* in, uint32_t inStride, uint8_t* out, uint32_t outStride,
                        uint32_t y, uint32_t rows, uint32_t height, const Kernel_t* kernel)
{
    ldlVerticalU16NEON(in, inStride, out, outStride, y, rows, height, kernel, 16383);
}
...
static const UpscaleHorizontal_t kHorizontalFunctionTable[ILCount][FPCount] = {
	...
	{ldlHorizontalU8PlanarNEON, ldlHorizontalU10PlanarNEON, ldlHorizontalU12PlanarNEON, ldlHorizontalU14PlanarNEON, ldlHorizontalS16PlanarNEON, ldlHorizontalS16PlanarNEON, ldlHorizontalS16PlanarNEON, ldlHorizontalS16PlanarNEON}, /* None*/
	{NULL,                      NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL},                       /* YUYV */
	{ldlHorizontalU8NV12NEON,   NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL},                       /* NV12 */
	{NULL,                      NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL},                       /* UYVY */
	{ldlHorizontalU8RGBNEON,    NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL},                       /* RGB */
	{ldlHorizontalU8RGBANEON,   NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL,                       NULL},                       /* RGBA */
};

/* kVerticalFunctionTable[fp] */
static const UpscaleVertical_t kVerticalFunctionTable[FPCount] = {
	ldlVerticalU8NEON,  /* U8 */
	ldlVerticalU10NEON, /* U10 */
	ldlVerticalU12NEON, /* U12 */
	ldlVerticalU14NEON, /* U14 */
	ldlVerticalS16NEON, /* S8.7 */
	ldlVerticalS16NEON, /* S10.5 */
	ldlVerticalS16NEON, /* S12.3 */
	ldlVerticalS16NEON, /* S14.1 */
};
...
UpscaleHorizontal_t ldlUpscaleGetHorizontalFunctionNEON(Interleaving_t ilv, FixedPoint_t srcFP,
                                                        FixedPoint_t dstFP, FixedPoint_t baseFP)
{
    /* Conversion is not currently supported in SIMD. */
    if ((srcFP != dstFP) || ((baseFP != dstFP) && ldlFixedPointIsValid(baseFP))) {
        return NULL;
    }
...
UpscaleVertical_t ldlUpscaleGetVerticalFunctionNEON(FixedPoint_t srcFP, FixedPoint_t dstFP)
{
    /* Conversion is not currently supported in SIMD. */
    if (srcFP != dstFP) {
        return NULL;
