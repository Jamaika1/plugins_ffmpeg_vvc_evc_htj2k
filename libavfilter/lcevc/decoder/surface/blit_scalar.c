...
static void copyIdentity(const BlitArgs_t* args)
{
    ...
    const uint32_t srcByteStride = src->stride * ldlFixedPointByteSize(src->type);
    const uint32_t dstByteStride = dst->stride * ldlFixedPointByteSize(dst->type);
...
BlitFunction_t surfaceBlitGetFunctionScalar(FixedPoint_t srcFP, FixedPoint_t dstFP, BlendingMode_t blending)
{
    if (blending == BMAdd) {
        ...
        if (srcFP != ldlFixedPointHighPrecision(dstFP)) {
            return NULL;
        }
    ...
    if (blending == BMCopy) {
        if ((srcFP == dstFP) || (ldlFixedPointIsSigned(srcFP) && ldlFixedPointIsSigned(dstFP))) {
            return &copyIdentity;
        }
