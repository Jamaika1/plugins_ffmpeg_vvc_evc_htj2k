...
static bool decodeResidualCoeffs(DecodeChunkArgs_t* args)
{
    ...
    while (tuIndex < tuCount) {
        int32_t run = ldlEntropyDecode(&decoder, &coeff);
...
static bool decodeTemporalCoeffs(DecodeChunkArgs_t* args)
{
    ...
    EntropyDecoder_t ldlEntropyDecoder = {0};
    if (entropyInitialise(args->log, &ldlEntropyDecoder, args->chunk, EDTTemporal,
                          args->bitstreamVersion) != 0) {
        return false;
    }

    while (tuIndex < tuState->tuTotal) {
        int32_t run = ldlEntropyDecodeTemporal(&ldlEntropyDecoder, &temporalSignal);
