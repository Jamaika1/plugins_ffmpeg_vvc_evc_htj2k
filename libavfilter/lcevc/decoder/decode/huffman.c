...
int32_t ldlHuffmanTripleInitialize(Logger_t log, HuffmanTripleDecodeState_t* state,
                                   HuffmanStream_t* stream, uint8_t bitstreamVersion)
{
    assert(state && stream);
...
int32_t ldlHuffmanManualDecode(const HuffmanManualDecodeState_t* state, HuffmanStream_t* stream,
                               uint8_t* symbolOut)
{
...
static int32_t ldlHuffmanManualDecodeMaybeSingleSymbol(const HuffmanManualDecodeState_t* state,
                                                       HuffmanStream_t* stream, uint8_t* symbolOut)
{
    ...
    return ldlHuffmanManualDecode(state, stream, symbolOut);
}

bool ldlHuffmanGetSingleSymbol(const HuffmanManualDecodeState_t* state, uint8_t* symbolOut)
{
    if (state->maxCodeLength + state->minCodeLength == 0) {
        *symbolOut = state->singleSymbol;
        return true;
...
int32_t ldlHuffmanLutDecode(const HuffmanTable_t* rlTable, HuffmanStream_t* stream, uint8_t* symbolOut)
{
...
/*- ldlHuffmanTripleDecodeState_t ------------------------------------------------------------------*/

int32_t ldlHuffmanTripleDecode(const HuffmanTripleDecodeState_t* state, HuffmanStream_t* stream,
                               int16_t* valueOut)
{
    ...
    if (lsbOverflowed(triplet.contents)) {
        VN_CHECK(ldlHuffmanManualDecodeMaybeSingleSymbol(&state->manualStates[HuffLSB], stream,
                                                         (uint8_t*)valueOut));
    ...
    if (nextSymbolIsMSB((uint8_t)*valueOut)) {
        uint8_t msb = 0;
        VN_CHECK(ldlHuffmanManualDecodeMaybeSingleSymbol(&state->manualStates[HuffMSB], stream, &msb));
    ...
    *valueOut >>= 1;
    ...
    while (seekRunLengths) {
        if (ldlHuffmanLutDecode(&state->rlTable, stream, &rlDetectionSymbol) < 0) {
            VN_CHECK(ldlHuffmanManualDecodeMaybeSingleSymbol(rlState, stream, &rlDetectionSymbol));
        }
