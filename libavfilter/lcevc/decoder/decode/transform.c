...
static inline void ldlDequantScalarImpl(const Dequant_t* dequant, TemporalSignal_t temporalSignal, int32_t numLayers,
...
void ldlDequantScalar(const Dequant_t* dequant, TemporalSignal_t temporalSignal, int32_t numLayers,
                      const int16_t* coeffs, int16_t* dequantizedCoeffs)
{
    ldlDequantScalarImpl(dequant, temporalSignal, numLayers, coeffs, dequantizedCoeffs);
}

/*------------------------------------------------------------------------------*/

static inline void ldlInverseDD1D(const int16_t* coeffs, int16_t* residuals)
{
...
void ldlDequantInverseDD1D(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                        const int16_t* coeffs, int16_t* residuals)
{
    int16_t dqCoeffs[RCLayerCountDD];
    ldlDequantScalarImpl(dequant, temporalSignal, RCLayerCountDD, coeffs, dqCoeffs);
    ldlInverseDD1D(dqCoeffs, residuals);
}

static inline void ldlInverseDD2D(const int16_t* coeffs, int16_t* residuals)
{
...
void ldlDequantInverseDD2D(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                        const int16_t* coeffs, int16_t* residuals)
{
    int16_t dqCoeffs[RCLayerCountDD];
    ldlDequantScalarImpl(dequant, temporalSignal, RCLayerCountDD, coeffs, dqCoeffs);
    ldlInverseDD2D(dqCoeffs, residuals);
}

void ldlInverseDDS1D(const int16_t* coeffs, int16_t* residuals)
{
    const int32_t a[4] =
    {
...
void ldlDequantInverseDDS1D(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                        const int16_t* coeffs, int16_t* residuals)
{
    int16_t dqCoeffs[RCLayerCountDDS];
    ldlDequantScalarImpl(dequant, temporalSignal, RCLayerCountDDS, coeffs, dqCoeffs);
    ldlInverseDDS1D(dqCoeffs, residuals);
}

void ldlInverseDDS2D(const int16_t* coeffs, int16_t* residuals)
{
    const int32_t a[4] =
    {
        coeffs[0]  + coeffs[1]  + coeffs[2]  + coeffs[3],
...
void ldlDequantInverseDDS2D(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                        const int16_t* coeffs, int16_t* residuals)
{
	int16_t dqCoeffs[RCLayerCountDDS];
	ldlDequantScalarImpl(dequant, temporalSignal, RCLayerCountDDS, coeffs, dqCoeffs);
    ldlInverseDDS2D(dqCoeffs, residuals);
}
...
static inline void ldlInverseDD1DImpl_SSE(__m128i ahvd, int16_t* residuals)
{
...
void ldlInverseDD1D_SSE(const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD1DImpl_SSE(_mm_loadl_epi64((const __m128i*)coeffs), residuals);
}

void ldlDequantInverseDD1D_SSE(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                            const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD1DImpl_SSE(dequantDD_SSE(dequant, temporalSignal, coeffs), residuals);
}

/*------------------------------------------------------------------------------*/

static inline void ldlInverseDD2DImpl_SSE(__m128i ahvd, int16_t* residuals)
{
    /* Re-order coefficients and negate some of them so we can
       add across all lanes and perform lane specific behavior.
...
void ldlInverseDD2D_SSE(const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD2DImpl_SSE(_mm_loadl_epi64((const __m128i*)coeffs), residuals);
}

void ldlDequantInverseDD2D_SSE(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                            const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD2DImpl_SSE(dequantDD_SSE(dequant, temporalSignal, coeffs), residuals);
}

/*------------------------------------------------------------------------------*/

static inline void ldlInverseDDS1DImpl_SSE(const __m128i coeffs[2], int16_t* residuals)
{
...
void ldlInverseDDS1D_SSE(const int16_t* coeffs, int16_t* residuals)
{
    const __m128i coeffsV[2] = { _mm_load_si128((const __m128i*)&coeffs[0]),
                                 _mm_load_si128((const __m128i*)&coeffs[8]) };
    ldlInverseDDS1DImpl_SSE(coeffsV, residuals);
}

void ldlDequantInverseDDS1D_SSE(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                             const int16_t* coeffs, int16_t* residuals)
{
    __m128i dqCoeffs[2];
    dequantDDS_SSE(dequant, temporalSignal, coeffs, dqCoeffs);
    ldlInverseDDS1DImpl_SSE(dqCoeffs, residuals);
}

/*------------------------------------------------------------------------------*/

void ldlInverseDDS2DImpl_SSE(const __m128i coeffs[2], int16_t* residuals)
{
    const __m128i a = _mm_cvtepi16_epi32(coeffs[0]);
    const __m128i h = _mm_cvtepi16_epi32(_mm_srli_si128(coeffs[0], 8));
    const __m128i v = _mm_cvtepi16_epi32(coeffs[1]);
...
void ldlInverseDDS2D_SSE(const int16_t* coeffs, int16_t* residuals)
{
    const __m128i coeffsV[2] = { _mm_load_si128((const __m128i*)&coeffs[0]),
                                 _mm_load_si128((const __m128i*)&coeffs[8]) };
    ldlInverseDDS2DImpl_SSE(coeffsV, residuals);
}

void ldlDequantInverseDDS2D_SSE(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                             const int16_t* coeffs, int16_t* residuals)
{
    __m128i coeffsV[2];
...
static inline void ldlInverseDD1DImpl_NEON(const int16x4_t coeffs, int16_t* residuals)
{
...
void ldlInverseDD1D_NEON(const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD1DImpl_NEON(vld1_s16(coeffs), residuals);
}

void ldlDequantInverseDD1D_NEON(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                            const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD1DImpl_NEON(dequantDD_NEON(dequant, temporalSignal, coeffs), residuals);
}

/*------------------------------------------------------------------------------*/

static inline void ldlInverseDD2DImpl_NEON(const int16x4_t coeffs, int16_t* residuals)
{
    const int32x4_t ahvd = vmovl_s16(coeffs);
    const int32x4_t hvda = vextq_s32(ahvd, ahvd, 1);
    const int32x4_t vdah = vextq_s32(ahvd, ahvd, 2);
...
void ldlInverseDD2D_NEON(const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD2DImpl_NEON(vld1_s16(coeffs), residuals);
}

void ldlDequantInverseDD2D_NEON(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                            const int16_t* coeffs, int16_t* residuals)
{
    ldlInverseDD2DImpl_NEON(dequantDD_NEON(dequant, temporalSignal, coeffs), residuals);
}

/*------------------------------------------------------------------------------*/

static inline void ldlInverseDDSPass2_NEON(int32x4x4_t* coeffs, int16_t* residuals)
{
    /* Transpose */
#if VN_ARCH(ARM7A)
...
static inline void ldlInverseDDS1DImpl_NEON(const int16x8_t coeffs[2], int16_t* residuals)
{
    ...
    ldlInverseDDSPass2_NEON(&firstPass, residuals);
}

void ldlInverseDDS1D_NEON(const int16_t* coeffs, int16_t* residuals)
{
    const int16x8_t coeffsV[2] = { vld1q_s16(&coeffs[0]),
                                   vld1q_s16(&coeffs[8]) };
    ldlInverseDDS1DImpl_NEON(coeffsV, residuals);
}

void ldlDequantInverseDDS1D_NEON(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                              const int16_t* coeffs, int16_t* residuals)
{
    int16x8_t dqCoeffs[2];
    dequantDDS_NEON(dequant, temporalSignal, coeffs, dqCoeffs);
    ldlInverseDDS1DImpl_NEON(dqCoeffs, residuals);
}

/*------------------------------------------------------------------------------*/

static inline void ldlInverseDDS2DImpl_NEON(const int16x8_t coeffs[2], int16_t* residuals)
{
    const int32x4_t a = vmovl_s16(vget_low_s16(coeffs[0]));
    const int32x4_t h = vmovl_s16(vget_high_s16(coeffs[0]));
    const int32x4_t v = vmovl_s16(vget_low_s16(coeffs[1]));
...
void ldlInverseDDS2D_NEON(const int16_t* coeffs, int16_t* residuals)
{
    const int16x8_t coeffsV[2] = { vld1q_s16(&coeffs[0]),
                                   vld1q_s16(&coeffs[8]) };
    ldlInverseDDS2DImpl_NEON(coeffsV, residuals);
}

void ldlDequantInverseDDS2D_NEON(const Dequant_t* dequant, TemporalSignal_t temporalSignal,
                              const int16_t* coeffs, int16_t* residuals)
{
    int16x8_t dqCoeffs[2];
    dequantDDS_NEON(dequant, temporalSignal, coeffs, dqCoeffs);
    ldlInverseDDS2DImpl_NEON(dqCoeffs, residuals);
}
...
static const TransformFunction_t kTable[2][2] = {{&ldlInverseDD2D, &ldlInverseDD1D},
                                                 {&ldlInverseDDS2D, &ldlInverseDDS1D}};

#if VN_CORE_FEATURE(SSE)

static const TransformFunction_t kTableSIMD[2][2] = {{&ldlInverseDD2D_SSE, &ldlInverseDD1D_SSE},
                                                     {&ldlInverseDDS2D_SSE, &ldlInverseDDS1D_SSE}};

#elif VN_CORE_FEATURE(NEON)

static const TransformFunction_t kTableSIMD[2][2] = {{&ldlInverseDD2D_NEON, &ldlInverseDD1D_NEON},
                                                     {&ldlInverseDDS2D_NEON, &ldlInverseDDS1D_NEON}};
...
TransformFunction_t ldlTransformGetFunction(TransformType_t transform, ScalingMode_t scaling,
...
static const DequantTransformFunction_t kDequantTable[2][2] = {
    {&ldlDequantInverseDD2D, &ldlDequantInverseDD1D},
    {&ldlDequantInverseDDS2D, &ldlDequantInverseDDS1D}};

#if VN_CORE_FEATURE(SSE)

static const DequantTransformFunction_t kDequantTableSIMD[2][2] = {
    {&ldlDequantInverseDD2D_SSE, &ldlDequantInverseDD1D_SSE},
    {&ldlDequantInverseDDS2D_SSE, &ldlDequantInverseDDS1D_SSE}};

#elif VN_CORE_FEATURE(NEON)

static const DequantTransformFunction_t kDequantTableSIMD[2][2] = {
    {&ldlDequantInverseDD2D_NEON, &ldlDequantInverseDD1D_NEON},
    {&ldlDequantInverseDDS2D_NEON, &ldlDequantInverseDDS1D_NEON}};
...
DequantTransformFunction_t ldlDequantTransformGetFunction(TransformType_t transform, ScalingMode_t scaling,
                                                          CPUAccelerationFeatures_t preferredAccel)
{
    const int32_t scalingIndex = (scaling == Scale1D) ? 1 : 0;
