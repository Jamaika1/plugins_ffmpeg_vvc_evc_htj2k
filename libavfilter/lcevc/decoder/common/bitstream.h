...
#ifndef VN_LCEVC_LEGACY_BITSTREAM_H
#define VN_LCEVC_LEGACY_BITSTREAM_H
...
int32_t ldlBitstreamReadBit(BitStream_t* stream, uint8_t* out);
...
void ldlBitstreamReadBitUnchecked(BitStream_t* stream, uint8_t* out);

/*! \brief read n bits from the stream.
 *
 *  \return <0 on failure */
int32_t ldlBitstreamReadBits(BitStream_t* stream, uint8_t numBits, int32_t* out);
...
static inline void loadWordUnchecked(BitStream_t* stream)
{
    ...
    if (remaining >= 4) {
        ...
        stream->byteStream.offset +=
            ldlReadU32(stream->byteStream.data + stream->byteStream.offset, &stream->word);
        stream->nextBit = 0;
    } else {
...
static inline void ldlBitstreamReadBitsUnchecked(BitStream_t* stream, uint8_t numBits, int32_t* out)
{
 ...
 *
 *  \return <0 on failure */
int32_t ldlBitstreamReadExpGolomb(BitStream_t* stream, uint32_t* out);

/*! \brief get number of remaining bits
 *
 *  \return number of bits remaining in stream */
static inline size_t bitstreamGetRemainingBits(const BitStream_t* stream)
{
...
static inline size_t bitstreamGetConsumedBits(const BitStream_t* stream)
{
    const size_t remainingBits = bitstreamGetRemainingBits(stream);
    const size_t overallSize = ldlByteStreamGetSize(&stream->byteStream) * 8;
...
static inline uint32_t bitstreamGetConsumedBytes(const BitStream_t* stream)
{
...
#endif // VN_LCEVC_LEGACY_BITSTREAM_H
