...
#ifndef VN_LCEVC_LEGACY_CMDBUFFER_H
#define VN_LCEVC_LEGACY_CMDBUFFER_H
 ...
 *    ldlCmdBufferAppend(buffer, x, y, values);
 *
 ...
 *    ldlCmdBufferAppendCoord(&buffer, x, y);
 *
 *    // When finished with a command buffer clean up.
 *    cmdBufferFree(buffer);
 *
...
typedef struct CmdBuffer
{
    ...
    CmdBufferStorage_t data; /**< Memory storage for commands and jumps from the start, residuals from the end. */
 ...
 * \return true on success, otherwise false.
 */
bool ldlCmdBufferAppend(CmdBuffer_t* cmdBuffer, CmdBufferCmd_t command, const int16_t* values,
                        uint32_t jump);
...
#endif // VN_LCEVC_LEGACY_CMDBUFFER_H
