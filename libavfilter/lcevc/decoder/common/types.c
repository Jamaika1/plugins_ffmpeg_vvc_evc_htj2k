...
{
    assert(ldlFixedPointIsValid(type));
...
bool ldlFixedPointIsValid(FixedPoint_t type) { return (uint32_t)type < FPCount; }
...
bool ldlIsPow2(uint32_t value) { return (value & (value - 1)) == 0; }

uint32_t ldlAlignTruncU32(uint32_t value, uint32_t alignment)
{
    assert(alignment > 0);
    assert(ldlIsPow2(alignment));
