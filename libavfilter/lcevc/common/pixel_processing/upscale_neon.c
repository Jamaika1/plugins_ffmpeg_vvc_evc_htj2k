...
void horizontalU8PlanarNEON(LdppDitherSlice* dither, const uint8_t* in[2], uint8_t* out[2],
                            ...
                            const LdeKernel* kernel, const LdpFixedPoint dstFP)
{
    ...
    if (upscaleHorizontalCoordsIsLeftValid(&coords)) {
        horizontalU8Planar(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel, dstFP);
    }
    ...
    if (upscaleHorizontalCoordsIsRightValid(&coords)) {
        horizontalU8Planar(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel, dstFP);
    }
}
...
void horizontalS16PlanarNEON(LdppDitherSlice* dither, const uint8_t* in[2], uint8_t* out[2],
                             const uint8_t* base[2], uint32_t width, uint32_t xStart, uint32_t xEnd,
                             const LdeKernel* kernel, LdpFixedPoint dstFP)
{
    const int16_t* kernelFwd = kernel->coeffs[0];
    const int16_t* kernelRev = kernel->coeffs[1];
    const int32_t kernelLength = (int32_t)kernel->length;
    ...
    /* Run left edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsLeftValid(&coords)) {
        horizontalS16Planar(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel, dstFP);
    }
    ...
    for (uint32_t x = coords.start; x < coords.end; x += UCHoriStepping) {
        ...
        if (ditherBuffer) {
            ldppDitherApplyNEON(&values[0], &ditherBuffer, shift, dither->strength);
            ldppDitherApplyNEON(&values[1], &ditherBuffer, shift, dither->strength);
        }
    ...
    /* Run right edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsRightValid(&coords)) {
        horizontalS16Planar(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel, dstFP);
    }
}

/*! \brief Planar horizontal upscaling of 2 rows. */
static inline void horizontalU16PlanarNEON(LdppDitherSlice* dither, const uint8_t* in[2],
...
/* Generate unsigned 2-byte wide planar upscale functions */
#define VN_HORI_MAX_VALUE_U10() , 1023, false
#define VN_HORI_MAX_VALUE_U12() , 4095, false
#define VN_HORI_MAX_VALUE_U14() , 16383, true

#define horizontalU16PlanarNEON(fp)                                                            \
    static void horizontal##fp##PlanarNEON(LdppDitherSlice* dither, const uint8_t* in[2],      \
                                           uint8_t* out[2], const uint8_t* base[2],            \
                                           uint32_t width, uint32_t xStart, uint32_t xEnd,     \
                                           const LdeKernel* kernel, const LdpFixedPoint dstFP) \
    {                                                                                          \
        horizontalU16PlanarNEON(dither, in, out, base, width, xStart, xEnd,                    \
                                kernel VN_HORI_MAX_VALUE_##fp());                              \
    }

horizontalU16PlanarNEON(U10);
horizontalU16PlanarNEON(U12);
horizontalU16PlanarNEON(U14);

/*! \brief NV12 horizontal upscaling of 2 rows. */
void horizontalU8NV12NEON(LdppDitherSlice* dither, const uint8_t* in[2], uint8_t* out[2],
                          const uint8_t* base[2], uint32_t width, uint32_t xStart, uint32_t xEnd,
                          const LdeKernel* kernel, const LdpFixedPoint dstFP)
{
    const int16_t* kernelFwd = kernel->coeffs[0];
    ...
    /* Run left edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsLeftValid(&coords)) {
        horizontalU8NV12(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel, dstFP);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsRightValid(&coords)) {
        horizontalU8NV12(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel, dstFP);
    }
}

/*! \brief RGB horizontal upscaling of 2 rows. */
void horizontalU8RGBNEON(LdppDitherSlice* dither, const uint8_t* in[2], uint8_t* out[2],
                         const uint8_t* base[2], uint32_t width, uint32_t xStart, uint32_t xEnd,
                         const LdeKernel* kernel, const LdpFixedPoint dstFP)
{
    const int16_t* kernelFwd = kernel->coeffs[0];
    ...
    /* Run left edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsLeftValid(&coords)) {
        horizontalU8RGB(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel, dstFP);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsRightValid(&coords)) {
        horizontalU8RGB(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel, dstFP);
    }
}

/*! \brief RGBA horizontal upscaling of 2 rows. */
void horizontalU8RGBANEON(LdppDitherSlice* dither, const uint8_t* in[2], uint8_t* out[2],
                          const uint8_t* base[2], uint32_t width, uint32_t xStart, uint32_t xEnd,
                          const LdeKernel* kernel, const LdpFixedPoint dstFP)
{
    uint32_t x;
    ...
    /* Run left edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsLeftValid(&coords)) {
        horizontalU8RGBA(dither, in, out, base, width, coords.leftStart, coords.leftEnd, kernel, dstFP);
    }
    ...
    /* Run right edge non-SIMD loop */
    if (upscaleHorizontalCoordsIsRightValid(&coords)) {
        horizontalU8RGBA(dither, in, out, base, width, coords.rightStart, coords.rightEnd, kernel, dstFP);
    }
}
...
static const UpscaleHorizontalFunction kHorizontalFunctionTable[ILCount][LdpFPCount] = {
